...
//    m_splitter->restoreState(Store::config.views.flowSize);
    ...
    connect(m_flow, &Flow::centerIndexChanged, this, &FlowView::flowCurrentIndexChanged);
    connect(m_flow, &Flow::opened, this, &FlowView::opened);
    connect(m_splitter, &QSplitter::splitterMoved, this, &FlowView::saveSplitter);
    connect(m_dView, &DetailsView::entered, this, &FlowView::entered);
    connect(m_dView, &DetailsView::opened, this, &FlowView::opened);
    connect(m_dView, &DetailsView::viewportEntered, this, &FlowView::viewportEntered);
    connect(m_dView, &DetailsView::newTabRequest, this, &FlowView::newTabRequest);
    connect(m_dView, &DetailsView::requestRightClickMenu, this, &FlowView::requestRightClickMenu);
}
...
FlowView::setModel(QAbstractItemModel *model)
{
    ...
    m_flow->setModel(static_cast<FS::ProxyModel *>(model));
...
FlowView::setSelectionModel(QItemSelectionModel *selectionModel)
{
    ...
    connect(selectionModel, &QItemSelectionModel::selectionChanged, this, &FlowView::treeCurrentIndexChanged);
...
{
    m_dView->scrollToTop();
    m_flow->showCenterIndex(rootIndex().child(0, 0));
}
...
FlowView::scrollToBottom()
{
    ...
    m_flow->showCenterIndex(rootIndex().child(model()->rowCount(rootIndex())-1, 0));
}
