    ...
    , m_animTimer(new QTimer(this))
{
    ...
    connect(m_animTimer, &QTimer::timeout, this, &Button::animate);
    m_animIcon = IconProvider::icon(IconProvider::Animator);
...
//    m_layout[Right]->addStretch();
    setMinimumHeight(32);

    m_layout[Main]->addLayout(m_layout[Left]);
    m_layout[Main]->addStretch();
    m_layout[Main]->addWidget(m_messageLbl, 100, Qt::AlignCenter);
    m_layout[Main]->addStretch();
...
QList<Configurable *> Configurable::s_configurables;

Configurable::Configurable()
{
    s_configurables << this;
}

Configurable::~Configurable()
{
    s_configurables.removeOne(this);
}

QList<Configurable *>
Configurable::configurables()
{ return s_configurables; }
